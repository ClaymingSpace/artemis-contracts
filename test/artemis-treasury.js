...
const ArtemisTreasury = artifacts.require("ArtemisTreasury")

contract("ArtemisTreasury", accounts => {
  const owners = [accounts[0], accounts[1], accounts[2]]
//  This becomes 7 in the case of the Artemis Accords as there are 13 nations signed on. 
//  Assuming this is a pure democratic vote with equal vote weighting 
//  For testing purposes we'll keep to three accounts with a 2/3 confirmation requirement
  const NUM_CONFIRMATIONS_REQUIRED = 2

  let wallet
  beforeEach(async () => {
    wallet = await ArtemisTreasury.new(owners, NUM_CONFIRMATIONS_REQUIRED)
  })

  describe("constructor", () => {
    it("should deploy", async () => {
      const wallet = await ArtemisTreasury.new(
        owners,
        NUM_CONFIRMATIONS_REQUIRED
      )

      // Tests that all addresses are equal to the owners
      for (let i = 0; i < owners.length; i++) {
        assert.equal(await wallet.nations(i), owners[i])
      }

    // Tests for equality in confirmations requirement
      assert.equal(
    ...
    // Rejects if no new owners added
    it("should reject if no owners", async () => {
      await expect(ArtemisTreasury.new([], NUM_CONFIRMATIONS_REQUIRED)).to.be
        .rejected
    })

    // self explainatory
    it("should reject if num conf required > owners", async () => {
      await expect(ArtemisTreasury.new(owners, owners.length + 1)).to.be.rejected
    })

    // unique owners test
    it("should reject if owners not unique", async () => {
      await expect(
        ArtemisTreasury.new([owners[0], owners[0]], NUM_CONFIRMATIONS_REQUIRED)
      ).to.be.rejected
    })
  })
...
//   Tests fallback function
  ...
  describe("getNations", () => {
    it("should return owners", async () => {
      const res = await wallet.getNations()
